...
func _ready() -> void:
	...
	for i in sounds.keys():
		sounds[i].stream = load("res://" + str(i) + ".mp3")
...
func install_sounds(node: Node) -> void:
	for i in node.get_children():
		if i is Button:
			i.pressed.connect( ui_sfx_play.bind(&"Skibidi") )
		elif i is OptionButton:
			i.pressed.connect( ui_sfx_play.bind(&"Skibidi") )
		elif i is TextureButton:
			i.pressed.connect( ui_sfx_play.bind(&"Skibidi") )
		elif i is TabContainer:
			i.tab_clicked.connect( ui_sfx_play.bind(&"Skibidi") )
		install_sounds(i)
